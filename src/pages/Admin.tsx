  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Kill switch global — visível em todas as abas relacionadas a envio */}
        {(tab === 'invites' || tab === 'logs') && (
          <div className={`mb-4 p-4 rounded-lg border-2 flex items-center justify-between gap-3 ${
            flowEnabled ? 'bg-green-500/10 border-green-500/40' : 'bg-destructive/10 border-destructive/40'
          }`}>
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {flowEnabled ? 'Envios pelo n8n estão ativos. Clique para pausar.' : 'Nenhum envio será disparado até religar manualmente.'}
              </p>
            </div>
            <button
              onClick={toggleFlow}
              ...
              className={`px-4 py-2 rounded-md font-medium text-sm transition disabled:opacity-50 ${
                flowEnabled ? 'bg-destructive text-destructive-foreground hover:opacity-90' : 'bg-green-600 text-white hover:opacity-90'
              }`}
            >
